...
// ------------------------------------------------------ 
    vector linear_predictor_S(matrix X, vector t_cf_event_q, int[] ID_q, int[] X_q, vector beta_tilde, real gamma1, real gamma2, real gamma3, real gamma4, matrix bi, real alpha){
      int N_q = num_elements(t_cf_event_q);
      vector[N_q] lp= beta_tilde[1] + beta_tilde[2]*t_cf_event_q + bi[ID_q,1] + bi[ID_q,2].*t_cf_event_q;
      vector[N_q] out = X[X_q,1]*gamma1 + X[X_q,2]*gamma2 + X[X_q,3]*gamma3 + X[X_q,4]*gamma4 + alpha*lp;
      return out;
    }
...
generated quantities {
  ...
  vector[N] llk_lg_all;
  for (i in 1:n) {
    for (j in 1:15) {cumhaz_q[j] = (T_e[i]/2)*wk[j]*(exp(eta_1*B_1_gq_q[i,j] + eta_2*B_2_gq_q[i,j] + eta_3*B_3_gq_q[i,j] + eta_4*B_4_gq_q[i,j] + eta_5*B_5_gq_q[i,j] + X[i,1]*gamma1 + X[i,2]*gamma2 + X[i,3]*gamma3 + X[i,4]*gamma4 + alpha*(beta_tilde[1] + beta_tilde[2]*T_e[i]*(1+xk[j])/2 + bi[i,1] + bi[i,2]*T_e[i]*(1+xk[j])/2)));}
    cumhaz[i] = sum(cumhaz_q);
    }
  for (i in 1:n) {log_haz[i] = T_status[i]*(eta_1*B_1_gq_haz[i] + eta_2*B_2_gq_haz[i] + eta_3*B_3_gq_haz[i] + eta_4*B_4_gq_haz[i] + eta_5*B_5_gq_haz[i] + X[i,1]*gamma1 + X[i,2]*gamma2 + X[i,3]*gamma3 + X[i,4]*gamma4 + alpha*(beta_tilde[1] + beta_tilde[2]*T_e[i] + bi[i,1] + bi[i,2]*T_e[i]));}
