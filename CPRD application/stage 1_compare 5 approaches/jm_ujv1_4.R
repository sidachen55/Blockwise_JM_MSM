...
data_UJ<-data_cr[sort(c(which((data_cr$transition==4) & (data_cr$status==1)),which((data_cr$transition==4) & (data_cr$status==0)))),]
...
data_UJ$transition<-replace(data_UJ$transition,data_UJ$transition==c(4),c(1))
...
#################################################################################
obs_median_transtime <- summary(data_mstate[data_mstate$transition == 4 & data_mstate$status == 1, ]$years)["Median"]
...
save(x, file=paste0("ujv1_4.rdata"))

pdf("trace_ujv1_4.pdf")
