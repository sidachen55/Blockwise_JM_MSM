...
data_cr$id<-rep(1:length(cr_id2),each=3)
...
max_transtime <- rep(NA,3)
for (i in 1:3) {max_transtime[i] <- summary(data_mstate[data_mstate$transition == i, ]$years)["Max."]}
obs_median_transtime <- rep(NA,3)
for (i in 1:3) {obs_median_transtime[i] <- summary(data_mstate[data_mstate$transition == i & data_mstate$status == 1, ]$years)["Median"]}
...
nchain<-1
fitLN1 <- stan(file = "stan_JM_cr_B1.stan", 
