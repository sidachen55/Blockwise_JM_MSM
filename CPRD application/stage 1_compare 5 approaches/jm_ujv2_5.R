...
data_UJ<-data_cr[sort(c(which((data_cr$transition==5) & (data_cr$status==1)),which((data_cr$transition==5) & (data_cr$status==0)))),]
...
data_UJ$transition<-replace(data_UJ$transition,data_UJ$transition==c(5),c(1))
...
#################################################################################
obs_median_transtime <- summary(data_mstate[data_mstate$transition == 5 & data_mstate$status == 1, ]$years)["Median"]
...
save(x, file=paste0("ujv2_5.rdata"))

pdf("trace_ujv2_5.pdf")
