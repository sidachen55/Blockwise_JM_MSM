...
#######################################
cr_id<-data_mstate$id[which(data_mstate$transition==7)]
data_cr<-data_mstate[sort(c(which(data_mstate$transition==6),which(data_mstate$transition==7))),]
data_UJ<-data_cr[sort(c(which((data_cr$transition==7) & (data_cr$status==1)),which((data_cr$transition==7) & (data_cr$status==0)))),]
...
data_UJ$transition<-replace(data_UJ$transition,data_UJ$transition==c(7),c(1))
...
#################################################################################
obs_median_transtime <- summary(data_mstate[data_mstate$transition == 7 & data_mstate$status == 1, ]$years)["Median"]
...
nchain<-1
fitLN1 <- stan(file = "stan_JM_ST.stan", 
               data = list(y=y,N=N,n=n,ID=ID,visits=visits,Nevents=Nevents,Qn=Qn,nrow_q=nrow_q,ID_q=ID_q,t_event_q=t_event_q,t_cf_event_q=t_cf_event_q,qwts=qwts,X=covdat,
...
save(x, file=paste0("ujv1_7.rdata"))

pdf("trace_ujv1_7.pdf")
