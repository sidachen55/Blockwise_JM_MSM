//------------------------------------------------------------------------------
// stan code fitting joint-single transition model (clock forward scale, updated 2024.3.10)
//------------------------------------------------------------------------------
functions{
// ------------------------------------------------------
...
// ------------------------------------------------------ 
    vector linear_predictor_S(matrix X, vector f1_2, vector f2_2, int[] ID_q, vector beta, vector beta_x, real gamma, matrix bi, real alpha){
      int N_q = num_elements(f1_2);
      vector[N_q] out;
 
      out = X[ID_q,1]*gamma + alpha*(beta[1] + beta_x[1]*X[ID_q,1] + bi[ID_q,1] + rows_dot_product(beta[2] + beta_x[2]*X[ID_q,1] + bi[ID_q,2],f1_2) + rows_dot_product(beta[3] + beta_x[3]*X[ID_q,1] + bi[ID_q,3],f2_2));
...
// ----- baseline log-hazard for a specific transition (here use weibull baseline)
    vector baseline_loghaz(vector B_1_event_q, vector B_2_event_q, vector B_3_event_q, vector B_4_event_q, vector B_5_event_q, real eta_1, real eta_2, real eta_3, real eta_4, real eta_5){
      ...
      out = eta_1*B_1_event_q + eta_2*B_2_event_q + eta_3*B_3_event_q + eta_4*B_4_event_q + eta_5*B_5_event_q;
...
// ------------------------------------------------------
{
 ...
 lp_s = linear_predictor_S(X, f1_2, f2_2, ID_q, beta, beta_x, gamma, bi, alpha);
