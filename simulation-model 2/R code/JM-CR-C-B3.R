...
data_cr <- data_mstate[sort(c(which(data_mstate$transition==8),which(data_mstate$transition==9))),]
...
data_lg <- do.call(rbind, L_data_split.by.id)
  ...
  data_cr$id<-rep(1:length(cr_id),each=2)
  data_cr$transition<-as.numeric(data_cr$transition)
  data_cr$transition<-replace(data_cr$transition,data_cr$transition==c(8:9),c(1:2))
  ...
  fitLN1 <- stan(file = "JM-CR-H_B3.stan", 
...
save(x, file=paste0("JM-CR-C_B3_",task_id,".rdata"))
