###################################################
# JM-CR-C for Ferrer's sim model 
# block 2 (TRANS=5,6,7)
###################################################
library(simsurv)
library(mvtnorm)
...
# data preparation 
################################################################################
cr_id <- data_mstate$id[which(data_mstate$transition==5)]
data_cr <- data_mstate[sort(c(which(data_mstate$transition==5),which(data_mstate$transition==6),which(data_mstate$transition==7))),]
L_data <- data.frame(ID=ID,long.data=longit.out,visits=visits.out)
L_data_cr<-L_data[L_data$ID %in% cr_id,]
L_data_cr$Tstop <- data_cr[data_cr$id %in% L_data_cr$ID, "Tstop"][match(L_data_cr$ID, data_cr$id)]
L_data_cr$Tstart <- data_cr[data_cr$id %in% L_data_cr$ID, "Tstart"][match(L_data_cr$ID, data_cr$id)]
...
data_lg <- do.call(rbind, L_data_split.by.id)
  ...
  data_cr$id<-rep(1:length(cr_id),each=3)
  data_cr$transition<-as.numeric(data_cr$transition)
  data_cr$transition<-replace(data_cr$transition,data_cr$transition==c(5:7),c(1:3))
  ...
  t_event_q<-vector()
  t_event_q<-data_cr$Tstop[which(data_cr$status==1)]
  ...
  fitLN1 <- stan(file = "JM-CR-H_B2.stan", 
...
save(x, file=paste0("JM-CR-C_B2_",task_id,".rdata"))
