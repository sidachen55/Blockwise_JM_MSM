// stan code fitting joint competing risk model
...
// ------------------------------------------------------
//     LINEAR PREDICTOR FOR THE competing risk SUBMODEL (time scale since Tstart)                
// ------------------------------------------------------ 
    vector linear_predictor_S(matrix X, vector t_cf_event_q, int[] ID_q, int[] trans_q, vector beta_tilde, vector gamma, matrix bi, vector alpha){
      int N_q = num_elements(t_cf_event_q);
      vector[N_q] out;
      vector[N_q] out0;
      out0 = beta_tilde[1] + beta_tilde[2]*t_cf_event_q + bi[ID_q,1] + rows_dot_product(bi[ID_q,2],t_cf_event_q);
...
// ----- baseline log-hazard for a specific transition (here use weibull baseline)
    vector baseline_loghaz(vector t_event_q, int[] trans_q, vector delta, vector lambda){
      int N_q = num_elements(t_event_q);
      vector[N_q] out;
      
      out = log(delta[trans_q])+log(lambda[trans_q])+rows_dot_product((delta[trans_q]-1),log(t_event_q));
...
data{
  ...
  int<lower=1,upper=3> trans_q[nrow_q];
...
parameters{
  ...
  real<lower=0> Var_e;  
  matrix[n,2] bi;
  vector<lower=0>[3] delta;
  vector<lower=0>[3] lambda;
  vector[3] alpha;
  vector[3] gamma;
  cholesky_factor_corr[2] L_Omega; // Cholesky factor of a 2x2 correlation matrix for Sigma
...
// ------------------------------------------------------
{
 ...
 log_basehaz = baseline_loghaz(t_event_q, trans_q, delta, lambda);
 
 lp_s = linear_predictor_S(X, t_cf_event_q, ID_q, trans_q, beta_tilde, gamma, bi, alpha);
